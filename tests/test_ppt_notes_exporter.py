...
class PptNotesExporterTest(unittest.TestCase):
    ...
    def test_content_filter_compacts_navigation_slides_by_default(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(template["review_depth"], "compressed")
            self.assertEqual(template["note_required_slide_count"], 2)
            prompt_pack = (workdir / "prompt_pack.md").read_text(encoding="utf-8")
            self.assertIn("Compression Contract", prompt_pack)
    ...
    def test_cli_creates_docx_and_quality_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((workdir / "study_pack" / "review_index.md").exists())
            ...
            self.assertTrue((deliverables / "10_阅读取舍.md").exists())
