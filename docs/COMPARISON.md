...
| --- | --- | --- | --- |
| Microsoft MarkItDown | Broad document-to-Markdown conversion, useful for LLM ingestion, plugin-friendly ecosystem | General converter; not a page-by-page teaching-note workflow with formula examples and quality gates | Keep Markdown output, but focus on slide pedagogy, screenshots, formulas, examples, and quality reports |
| pptx2md | Focused PPTX-to-Markdown converter with images and slide text extraction | Primarily conversion; no Word notes, no explanation schema, no formula/example quality gate | Provide Markdown plus DOCX, prompt pack, per-slide explanation schema, and validation |
